...
from ..core.projects import append_log_line, emit_stage_progress, update_stage_detail, update_state
...
def try_run_pycolmap_global_mapping(project_id, paths, config, colmap_cfg, num_images):
    ...
    try:
        ...
        update_stage_detail(project_id, 'sparse_reconstruction', text='Initializing experimental pycolmap global mapping...', subtext=f'{num_images} images')
        emit_stage_progress(
            project_id,
        ...
        append_log_line(project_id, f"⚠️ pycolmap.global_mapping failed, falling back to CLI backend: {exc}")
        return False


def run_sparse_reconstruction_stage(project_id, paths, config, colmap_config=None, *, helpers):
    ...
    colmap_cfg, _ = apply_no_regression_floor(colmap_cfg, project_id=project_id, reason='before sparse reconstruction')
    ...
    use_pycolmap_global = sfm_backend == 'pycolmap' and use_global_sfm and not use_legacy_glomap
    fastmap_temp_dir = None

    prefer_incremental_sfm, incremental_reason = helpers['should_prefer_incremental_sfm'](config, paths, num_images)
    ...
    update_stage_detail(project_id, 'sparse_reconstruction', text='Initializing...', subtext=f'{num_images} images')
    ...
    if use_fastmap:
        ...
        if config.get('fast_sfm', False):
            cmd.extend(['--ba_iteration_num', '2', '--retriangulation_iteration_num', '0'])
            append_log_line(project_id, "⚡ Fast SfM mode: reduced iterations for speed")
        append_log_line(
            project_id,
            f"🔧 Legacy GLOMAP path: {GLOMAP_PATH}" if use_legacy_glomap else f"🔧 Using COLMAP executable for global mapper: {colmap_exe}",
        )
    else:
    ...
    glomap_stages = {
        ...
        'view_graph_calibration': {'progress': 10, 'label': '📊 View Graph Calibration'},
    ...
    def sparse_line_handler(line):
        ...
        if use_fastmap:
            for stage_key, pattern in [
                ...
                if re.search(pattern, line_lower):
                    ...
                    details = {'text': stage_info['label'], 'current_item': progress, 'total_items': 100, 'item_name': stage_key, 'fastmap_stage': stage_key, 'sfm_engine': 'fastmap'}
                    ...
                    update_stage_detail(project_id, 'sparse_reconstruction', text=stage_info['label'], subtext=f'FastMap - {num_images} images')
                    return
            return
        ...
        if use_global_sfm:
            for stage_key, pattern in [
                ...
                if re.search(pattern, line_lower):
                    ...
                    details = {'text': stage_info['label'], 'current_item': progress, 'total_items': 100, 'item_name': stage_key, 'glomap_stage': stage_key, 'sfm_engine': 'glomap'}
                    emit_stage_progress(project_id, 'sparse_reconstruction', progress, details)
                    update_state(project_id, 'sparse_reconstruction', progress=progress, details=details)
                    update_stage_detail(project_id, 'sparse_reconstruction', text=stage_info['label'], subtext=f'GLOMAP - {num_images} images')
            ...
            if relpose_match:
                ...
                details = {'text': f'📐 Relative Pose: {rel_percent}%', 'current_item': rel_percent, 'total_items': 100, 'item_name': f'{rel_percent}%', 'glomap_stage': 'relative_pose', 'sfm_engine': 'glomap'}
                emit_stage_progress(project_id, 'sparse_reconstruction', progress, details)
                update_state(project_id, 'sparse_reconstruction', progress=progress, details=details)
                update_stage_detail(project_id, 'sparse_reconstruction', text=f'📐 Relative Pose Estimation: {rel_percent}%', subtext=f'GLOMAP - {num_images} images')
                return
            ...
            if track_match:
                ...
                details = {'text': f'🔗 Tracks: {current_track}/{total_tracks}', 'current_item': current_track, 'total_items': total_tracks, 'item_name': f'Track {current_track}', 'glomap_stage': 'track_establishment', 'sfm_engine': 'glomap'}
                emit_stage_progress(project_id, 'sparse_reconstruction', progress, details)
                update_state(project_id, 'sparse_reconstruction', progress=progress, details=details)
                update_stage_detail(project_id, 'sparse_reconstruction', text=f'🔗 Track Establishment: {current_track}/{total_tracks}', subtext=f'GLOMAP - {track_percent}%')
                return
            ...
            if ba_match:
                ...
                details = {'text': f'⚡ Bundle Adjustment: {ba_current}/{ba_total}', 'current_item': ba_current, 'total_items': ba_total, 'item_name': f'Iteration {ba_current}', 'glomap_stage': 'bundle_adjustment', 'sfm_engine': 'glomap'}
                emit_stage_progress(project_id, 'sparse_reconstruction', progress, details)
                update_state(project_id, 'sparse_reconstruction', progress=progress, details=details)
                update_stage_detail(project_id, 'sparse_reconstruction', text=f'⚡ Bundle Adjustment: Iteration {ba_current}/{ba_total}', subtext=f'GLOMAP - {ba_percent}%')
                ba_log_state = {'last_milestone': sparse_tracker.get('last_ba_milestone', -1)}
                should_log, _ = should_emit_progress_milestone(ba_log_state, ba_current, ba_total, percent_step=25)
            ...
            if pair_match:
                ...
                if current_pair % 500 == 0 or current_pair == total_pairs:
                    details = {'text': f'🔧 Loading pairs: {current_pair}/{total_pairs}', 'current_item': current_pair, 'total_items': total_pairs, 'item_name': f'Pair {current_pair}', 'glomap_stage': 'preprocessing', 'sfm_engine': 'glomap'}
                    emit_stage_progress(project_id, 'sparse_reconstruction', progress, details)
                    update_stage_detail(project_id, 'sparse_reconstruction', text=f'🔧 Loading Image Pairs: {current_pair}/{total_pairs}', subtext='GLOMAP - Preprocessing')
                return
        ...
        for pattern in [
            ...
            if match:
                ...
                details = {'text': f'Images registered: {current}/{total}', 'current_item': current, 'total_items': total, 'item_name': f'Image {current}', 'sfm_engine': 'colmap'}
                ...
                update_stage_detail(project_id, 'sparse_reconstruction', text=f'Images registered: {current}/{total}', subtext='COLMAP')
                registration_log_state = {'last_milestone': sparse_tracker.get('last_registration_milestone', -1)}
                should_log, progress_percent = should_emit_progress_milestone(registration_log_state, current, total)
                sparse_tracker['last_registration_milestone'] = registration_log_state['last_milestone']
                if should_log:
                    append_log_line(project_id, f"[COLMAP] Registration progress: {current}/{total} images ({progress_percent}%)")
                return

    pycolmap_completed = False
    if use_pycolmap_global:
        pycolmap_completed = try_run_pycolmap_global_mapping(project_id, paths, config, colmap_cfg, num_images)
    if not pycolmap_completed:
        run_command_with_logs(project_id, cmd, line_handler=sparse_line_handler, raw_line_filter=should_log_subprocess_line)
    ...
    if use_fastmap:
        ...
        engine_name = "pycolmap Global Mapper" if use_pycolmap_global and pycolmap_completed else ("Legacy GLOMAP" if use_legacy_glomap else "COLMAP Global Mapper")
    else:
        engine_name = "COLMAP"
    ...
    sparse_summary = helpers['report_sparse_model_coverage'](project_id, paths, config, colmap_cfg, num_images)
    if sparse_summary:
        colmap_cfg['last_sparse_summary'] = dict(sparse_summary)
        colmap_cfg['no_regression_floor'] = merge_no_regression_floors(colmap_cfg.get('no_regression_floor'), capture_no_regression_floor(colmap_cfg))

    if helpers['should_run_boundary_frame_densification'](config, colmap_cfg, sparse_summary, paths):
        densified_result = helpers['run_boundary_frame_densification_recovery'](project_id, paths, config, colmap_cfg)
        if densified_result is not None:
            return densified_result

    densified_overlap_retry_pass = helpers['build_densified_overlap_retry_pass'](paths, colmap_cfg, sparse_summary)
    if densified_overlap_retry_pass:
        ...
        append_log_line(project_id, "🧠 Sparse reconstruction is still split after boundary densification; running a data-driven overlap retry with the standard sequential matcher")
        append_log_line(project_id, f"🧠 Densified-set overlap retry: {overlap_plan.get('current_overlap', '?')}→{overlap_plan.get('target_overlap', '?')} (cap={overlap_plan.get('overlap_cap', '?')}, boost={overlap_plan.get('overlap_boost', '?')})")
        helpers['clear_sparse_reconstruction_outputs'](paths['sparse_path'])
        colmap_cfg = helpers['run_orbit_safe_bridge_recovery_matching_pass'](project_id, paths, config, colmap_exe, colmap_cfg, has_cuda)
        return run_sparse_reconstruction_stage(project_id, paths, config, colmap_cfg, helpers=helpers)

    if helpers['should_run_final_loop_detection_recovery'](colmap_cfg, sparse_summary, num_images):
        ...
        append_log_line(project_id, "🧠 Sparse reconstruction is still split after overlap-only recovery; running final loop-detection fallback and retrying sparse reconstruction once")
        helpers['clear_sparse_reconstruction_outputs'](paths['sparse_path'])
        colmap_cfg = helpers['run_orbit_safe_bridge_recovery_matching_pass'](project_id, paths, config, colmap_exe, colmap_cfg, has_cuda)
        return run_sparse_reconstruction_stage(project_id, paths, config, colmap_cfg, helpers=helpers)
...
def run_model_conversion_stage(project_id, paths, *, helpers):
    ...
    sparse_model_path = helpers['select_best_sparse_model'](paths['sparse_path'], project_id)
