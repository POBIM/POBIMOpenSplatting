    ...
    *,
    ...
    try:
        ...
        def training_line_handler(line):
            ...
            wrote_match = re.search(r"Wrote\s+(.+\.ply)$", line)
            if wrote_match:
                written_path = Path(wrote_match.group(1).strip())
                if written_path.exists():
                    final_output_path = output_ply.resolve()
                    written_resolved = written_path.resolve()
                    preview_iteration_match = re.search(
                        rf"_(\d+){re.escape(output_ply.suffix)}$",
                        written_path.name,
                        re.IGNORECASE,
                    )
                    if written_resolved == final_output_path:
