...
class PipelineStageSmokeTests(unittest.TestCase):
    ...
    def test_opensplat_anti_fog_presets_expose_runtime_knobs(self):
        ...
        self.assertEqual(small_fog_heavy['iterations'], 1500)
        self.assertEqual(small_fog_heavy['target_visits_per_image'], 50)
        self.assertEqual(small_fog_heavy['refine_every'], 66)
        self.assertEqual(small_fog_heavy['warmup_length'], 360)
        self.assertEqual(small_fog_heavy['resolution_schedule'], 360)
        self.assertEqual(small_fog_heavy['stop_screen_size_at'], 480)

        self.assertEqual(fog_heavy['iterations'], 3900)
        self.assertEqual(fog_heavy['densify_grad_threshold'], 0.00055)
        self.assertEqual(fog_heavy['refine_every'], 172)
        self.assertEqual(fog_heavy['warmup_length'], 936)
        self.assertEqual(fog_heavy['ssim_weight'], 0.06)
        self.assertEqual(fog_heavy['num_downscales'], 1)
        self.assertEqual(fog_heavy['resolution_schedule'], 936)
        self.assertEqual(fog_heavy['split_screen_size'], 0.022)
        self.assertEqual(fog_heavy['stop_screen_size_at'], 1248)

        self.assertEqual(production_balanced['iterations'], 3900)
        self.assertEqual(production_balanced['densify_grad_threshold'], 0.00042)
        self.assertEqual(production_balanced['refine_every'], 140)
        self.assertEqual(production_balanced['warmup_length'], 702)
        self.assertEqual(production_balanced['ssim_weight'], 0.10)
        self.assertEqual(production_balanced['split_screen_size'], 0.03)
        self.assertEqual(production_balanced['stop_screen_size_at'], 1560)

        large_production = config_builders.get_opensplat_runtime_recommendation(
            'production',
            150,
        )
        self.assertEqual(large_production['iterations'], 7500)
        self.assertEqual(large_production['refine_every'], 270)
        self.assertEqual(large_production['warmup_length'], 1350)
        self.assertEqual(large_production['resolution_schedule'], 1800)
        self.assertEqual(large_production['stop_screen_size_at'], 3000)

        high_budget = config_builders.get_opensplat_runtime_recommendation(
            'production',
            150,
            {'target_visits_per_image': 125},
        )
        self.assertEqual(high_budget['iterations'], 18800)
        self.assertEqual(high_budget['target_visits_per_image'], 125)
