            ...
            if (line.rfind(pattern, 0) == 0){
                ...
                if (step >= 0){
                    ...
                    if (line.rfind(pattern, 0) == 0){
                        ...
                        const std::streamoff bytesRead = f.tellg();
                        if (bytesRead < 0) {
                            throw std::runtime_error("Unable to determine PLY header size");
                        }

                        const size_t bytesPerPoint = sizeof(float) * (14 + featuresDcSize + featuresRestSize);
                        const size_t remainingFileSize = fs::file_size(filename) - static_cast<size_t>(bytesRead);
